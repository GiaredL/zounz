...
const HeroSection: FC<HeroSectionProps> = ({
  ...
  const [selectedTestArtist, setSelectedTestArtist] =
    useState<ITestUsers | null>(null);

  const handleArtistFound = (artist: ArtistData | null) => {
    setSpotifyArtist(artist);
    if (artist?.name) {
      const filteredArtists = testUsers.filter((testArtist) =>
        testArtist.name?.toLowerCase().includes(artist.name.toLowerCase())
      );
      setSelectedTestArtist(filteredArtists[0] || null);
    } else {
        ...
        <div className={styles.artist}>
          <div className={styles.artistDetails}>
            <div
              ...
              style={{
                backgroundImage: `url(${spotifyArtist.images[0]?.url})`,
              }}
            >
              ...
              {selectedTestArtist?.songs ? (
                selectedTestArtist.songs.map((song) => (
                  <SongItem
                    ...
                    selectedArtist={selectedTestArtist}
                    song={song}
                  />
                ))
              ) : (
